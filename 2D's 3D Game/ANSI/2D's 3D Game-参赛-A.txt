...
bool cantgoORnot(char _ch,int _x,int _y)//判断是否能走
{
	if(_ch=='w')
	...
	for(f=1;f<=m;f++)//输出地图
	{
		for(s=1;s<=n;s++)
		{
			...
			if(_o)//多彩模式
			{
				...
				if(ma>9)//修改方块如果超过9，启用备用配色吗
				{
					if(a[f][s]>9)//9以上的cll用最高最高点的配色
					{
...
bool Keyword(string _a)//判定按键是否为关键字，只要给一个一连串的关键字集_a,就能识别
{
	int l=_a.size(),f;
	for(f=0;f<l;f++)
	{
